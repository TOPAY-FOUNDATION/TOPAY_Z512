...
async function fragmentationExample(): Promise<void> {
  ...
  try {
    ...
    const dataIntegrity = testData.length === reconResult.data.length &&
    ...
    const { timeMs: parallelReconTime, result: parallelReconResult } = await measureTime(async () => {
    ...
    const { timeMs: sequentialReconTime, result: sequentialReconResult } = await measureTime(async () => {
      return await reconstructData(parallelResult[0]?.fragments || []);
    });
    
    console.log(`   Sequential reconstruction time: ${sequentialReconTime}ms`);
    console.log(`   Performance improvement: ${(sequentialReconTime / parallelReconTime).toFixed(2)}x`);
    ...
    const serializationValid = deserialized.length === fragResult.fragments.length &&
                              deserialized.every((frag, index) => 
                                frag.index === fragResult.fragments[index]!.index);
    ...
    const originalSize = patternFragResult.fragments.reduce((sum, frag) => sum + frag.data.length, 0);
    ...
    console.log(`   Partial reconstruction: ${partialReconResult.isComplete ? 'COMPLETE' : 'INCOMPLETE'}`);
    ...
    for (const size of sizes) {
      ...
      console.log(`   ${size.toString().padStart(12)} | ${fragTime.toString().padStart(14)} | ${reconTime.toString().padStart(15)} | ${perfFragResult.metadata.fragmentCount.toString().padStart(9)}`);
    }
