...
async function keyPairExample(): Promise<void> {
  ...
  try {
    ...
    const keysMatch =
      toHex(deterministicKeyPair1.privateKey) === toHex(deterministicKeyPair2.privateKey);
    ...
    const passwordKeysMatch =
      toHex(passwordKeyPair1.privateKey) === toHex(passwordKeyPair2.privateKey);
