...
async function kemExample(): Promise<void> {
  ...
  try {
    ...
    const aliceSharedSecret = await kemDecapsulate(aliceKeyPair.secretKey, bobEncapResult.ciphertext);
    ...
    const serializationValid = toHex(testKemKeyPair.secretKey) === toHex(deserialized.secretKey) &&
    ...
    const isErased = memTestKeyPair.secretKey.every(byte => byte === 0) &&
