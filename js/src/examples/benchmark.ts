...
async function benchmarkExample(): Promise<void> {
  ...
  try {
    ...
    console.log(`   Operations tested: ${hashBenchmark.throughputOpsPerSec?.toLocaleString() || 'N/A'}/sec`);
    ...
    console.log(`   Key pairs/sec: ${keyPairBenchmark.throughputOpsPerSec?.toLocaleString() || 'N/A'}`);
    ...
    console.log(`   KEM operations/sec: ${kemBenchmark.throughputOpsPerSec?.toLocaleString() || 'N/A'}`);
    ...
    console.log(`   Fragmentation ops/sec: ${fragmentBenchmark.throughputOpsPerSec?.toLocaleString() || 'N/A'}`);
    ...
    console.log(`   Average time per benchmark: ${suiteResults.summary.averageTimeMs.toFixed(2)}ms`);
    console.log(`   Overall throughput: ${suiteResults.summary.operationsPerSecond.toFixed(2)} ops/sec`);
    ...
    console.log(`   Profile duration: ${(cpuProfile.cpuMetrics.samples.length * 100).toFixed(2)}ms`);
    ...
    console.log(`   Estimated throughput: ${mobilePerf.throughputOpsPerSec?.toFixed(1) || 'N/A'} ops/sec`);
    ...
    for (const { name, op } of operations) {
      ...
      console.log(`   ${name.padEnd(19)} | ${timeMs.toFixed(3).padStart(9)} | ${(memAfter - memBefore).toFixed(2).padStart(10)}`);
    }
    ...
    for (const size of dataSizes) {
      ...
      console.log(`   ${(size / 1024).toString().padStart(13)} | ${fragTime.toFixed(3).padStart(14)} | ${reconTime.toFixed(3).padStart(15)} | ${throughput.toFixed(2).padStart(16)}`);
    }
