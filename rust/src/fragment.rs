...
impl FragmentEngine {
    ...
    pub fn fragment_data(data: &[u8]) -> Result<Vec<Fragment>> {
        if data.is_empty() {
            return Err(TopayzError::FragmentationError(
                "Cannot fragment empty data".to_string(),
            ));
        }
    ...
    pub fn process_fragmented_kem(fragmented_result: &FragmentedResult) -> Result<Vec<u8>> {
        ...
        let total_size: usize = fragmented_result
            .fragments
            .iter()
            .map(|f| f.data.len())
            .sum();
...
impl Fragment {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < 76 {
            // 12 + 64 minimum
            return Err(TopayzError::FragmentationError(
                "Invalid fragment bytes".to_string(),
            ));
        }
        ...
        if bytes.len() != 12 + data_len + 64 {
            return Err(TopayzError::FragmentationError(
                "Fragment size mismatch".to_string(),
            ));
        }
        ...
        if !fragment.verify_fast() {
            return Err(TopayzError::FragmentationError(
                "Fragment integrity check failed".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_fragmented_kem() {
        ...
        assert_eq!(
            fragmented.fragments[0].total as usize,
