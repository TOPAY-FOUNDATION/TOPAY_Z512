...
fn main() {
    ...
    println!("   Private key (first 16 chars): {}", &keypair.private_key().to_hex()[..16]);
    println!("   Public key (first 16 chars):  {}\n", &keypair.public_key().to_hex()[..16]);
    ...
    println!("   Keys match: {}\n", 
        *keypair.private_key() == restored_private && 
        *keypair.public_key() == restored_public
    );

    println!("🎉 Quick start complete! You're ready to use TOPAY-Z512.");
    println!("\nNext steps:");
    println!("- Run other examples: cargo run --example [keypair_example|kem_example|hash_example]");
