...
fn main() -> Result<()> {
    ...
    #[cfg(feature = "fragmentation")]
    {
        ...
        println!("   Fragment size: {} bytes each", topayz512::fragment::FRAGMENT_SIZE);
        ...
        println!("   Deserialization success: {}", test_fragment == deserialized);
        ...
        if FragmentEngine::should_fragment(blockchain_data.len()) {
            println!("   Data size: {} bytes - fragmenting for parallel processing", blockchain_data.len());
            ...
            println!("   Data size: {} bytes - too small for fragmentation", blockchain_data.len());
        }
        println!();
