...
fn main() {
    ...
    println!(
        ...
        &public_key.to_bytes()[..32]
    );
    println!(
        "   Secret Key (first 32 bytes): {:02x?}",
        &secret_key.to_bytes()[..32]
    );
    println!();
    ...
    println!(
        ...
        shared_secret1.to_bytes().len()
    );
    println!(
        "   Ciphertext (first 32 bytes): {:02x?}",
        &ciphertext.to_bytes()[..32]
    );
    ...
    println!(
        "   Secrets Match: {}",
        shared_secret1.to_hex() == shared_secret2.to_hex()
    );
    println!();
    ...
    println!(
        "   Ciphertext A (first 16 bytes): {:02x?}",
        &ciphertext_a.to_bytes()[..16]
    );
    println!(
        "   Ciphertext B (first 16 bytes): {:02x?}",
        &ciphertext_b.to_bytes()[..16]
    );
    println!(
        "   Ciphertexts Different: {}",
        ciphertext_a.to_bytes() != ciphertext_b.to_bytes()
    );
    ...
    println!(
        "   Shared Secret Hex Length: {} chars",
        shared_secret_hex.len()
    );
    println!();
    ...
    let shared_secret_restored =
        topayz512::kem::SharedSecret::from_hex(&shared_secret_hex).unwrap();

    println!(
        "   Public Key Match: {}",
        public_key.to_bytes() == public_key_restored.to_bytes()
    );
    println!(
        "   Secret Key Match: {}",
        secret_key.to_bytes() == secret_key_restored.to_bytes()
    );
    println!(
        "   Ciphertext Match: {}",
        ciphertext.to_bytes() == ciphertext_restored.to_bytes()
    );
    println!(
        "   Shared Secret Match: {}",
        shared_secret1.to_bytes() == shared_secret_restored.to_bytes()
    );
    println!();
